...
| `flag` | string \| `true` | no | **Optional.** A problem / bug marker — draws a small red bug on the node's **top-right corner**; a string value shows as a hover note. Draw-only (does not affect layout or canvas bounds). See [§ Problem marker](#problem-marker-flag). |
